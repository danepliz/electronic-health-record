...
class MemberRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function listMembers($offset = NULL, $perPage = NULL, $filters = []){
        ...
        if(array_key_exists('name', $filters) and $filters['name'] != ''){
            $qb->andWhere(
                $qb->expr()->orX(
                    $qb->expr()->like('m.firstName', $qb->expr()->literal('%'.$filters['name'].'%')),
                    $qb->expr()->like('m.middleName', $qb->expr()->literal('%'.$filters['name'].'%')),
                    $qb->expr()->like('m.lastName', $qb->expr()->literal('%'.$filters['name'].'%'))
                )
            );
        }
...
//            $qb->andWhere('TIMESTAMPDIFF(SECOND, m.dob, '.$now.') >= :ageMin')->setParameter('ageMin', $filters['age_min']);
//            $qb->andWhere($qb->expr()->diff('m.dob', $today).' >= :ageMin')->setParameter('ageMin', $filters['age_min']);
            $qb->andWhere('DATE_DIFF(m.dob, CURRENT_DATE()) >= :ageMin')->setParameter('ageMin', $filters['age_min']);
